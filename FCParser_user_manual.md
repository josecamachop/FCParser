...
**Output:** In this field, the output directory for the parsed data and the stats file (which contains lines, records, matches)
are defined. Header.dat (containing a list of feature names) and weights.dat files are also generated automatically in this directory.

**Deparsing_output:** In this field the output directory for the _deparsed_ raw data and the
stats file are defined. This configuration is only used for _deparsing_ process. In addition,
the upper limit of log entries by datasource can be specified in the threshold parameter.

**Split:** In this field, the temporal sampling parameters are specified. Time window in
minutes, as well as start time and end time for sampling interval. Time parameters format is YYYY-MM-DD hh:mm:ss.
Output directory for split data can also be specified here.
...
- **Max_chunck**: Size (in kilobytes) of the chunk of files that are being processed at the same time.
...
- _Timestamp_regexp_: regular expresion that matches the date format of the source
    (only needed for unstructured sources).
- _Separator_: Char that delimits the log entries of the source (only needed in
    unstructured sources).
- _Timearg_: position of the timestamp in the log entry (only needed in structured
    sources). Timearg is the name of the timestamp variable.
...
- _Matchytype_ is the type of the field extracted (e.g string, number, ip, etc).
...
Note: There must be a variable called timestamp that collects the timestamps as strings.
